...
export default function Inventory() {
  ...
  const handleTableCellClick = (row) => {
    ...
    switch (row.integrationProvider) {
      case "AWS":
        idToSearch = row.integrationConfig[0].accountId;
        break;
      case "Azure":
        idToSearch = row.integrationConfig[0].subscriptionId;
        break;
      case "Google":
        idToSearch = row.integrationConfig[0].project;
        break;
      default:
        break;
    }

    // Construct the URL with the idToSearch
    const url = `http://localhost:8000/v1/collection/discoveryscandata?param=${idToSearch}`;
    ...
    fetch(url)
      ...
      .then((data) => {
        // Handle the JSON data here, e.g., set a state or display the result.
        console.log(data);
      })
      .catch((error) => {
        // Handle errors here.
  ...
  const dataToSend =
    { id: "64f8509607e7a348bcbfc0f2", menuname: "integrationdata" } || {};
      ...
      <div className="main-card mb-3 card">
        ...
        <div className="mb-3 mt-3 text-center">
          ...
          .fixed-table-header {
            position: sticky;
            top: 0;
            background-color: #3f6ad7; /* Adjust this to match your table's header background color */
            z-index: 1;
          }
            ...
            <table>
              <thead>
                <tr
                  ...
                  className="fixed-table-header" // Add this class to the header row
                >
                  {gethead.length > 0 ? (
                    <>
                      ...
                      <th className="text-center">Configuration</th>
                      <th className="text-center">Status</th>
                      <th className="text-center">Scan Report</th>
                    </>
                  ) : (
                    <>
              ...
              <TableBody>
                {getData && getData.length > 0 ? (
                  getData.map((row) => (
                    ...
                    >
                      <TableCell>{row.integrationProvider}</TableCell>

                      <TableCell>{row.integrationTitle}</TableCell>

                      <TableCell>{row.integrationDescription}</TableCell>
                      <TableCell>
                        {row.integrationProvider === "AWS"
                          ? row.integrationConfig[0].accountId
                          : row.integrationProvider === "Azure"
                          ? row.integrationConfig[0].subscriptionId
                          : row.integrationProvider === "Google"
                          ? row.integrationConfig[0].project
                          : ""}
                      </TableCell>
