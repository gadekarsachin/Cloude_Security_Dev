...
export default function Evidence() {
  ...
  function searchColletion(data) {
    ...
    fetch(
      ` /${dataToSend.menuname}?page=${currentPage}&limit=${limit}&keyword=${data}`,
      requestOptions
    )
      .then((response) => response.text())
      .then((result) => {
  ...
  function createEvidanceTask() {
    navigate("/audit-evidence", {
      state: { selectedAuditId: selectedAuditId }, // Fix the closing parenthesis
    });
  }
                ...
                {getData && getData.length > 0 ? (
                  getData.map((row, index) => (
                    ...
                    >
                      ...
                      >
                        {expandedTitle[index] ||
                        row.TaskRequirements.length <= 40
                          ? row.TaskRequirements
                          : `${row.TaskRequirements.slice(0, 40)} ... `}
                        {row.TaskRequirements.length > 40 && (
                          <button
                            style={{
                          ...
                          >
                            {expandedTitle[index] ? "Show Less" : "Show More"}
                          </button>
                        )}
                      </TableCell>
                      <TableCell>{row.TaskAssignee}</TableCell>
                      <TableCell>{row.TaskStatus}</TableCell>
                      <TableCell>{row.TaskCreatedOn}</TableCell>
                      <TableCell>{row.TaskCreatedBy}</TableCell>
                      <TableCell>{row.TaskCompletedOn}</TableCell>
                      <TableCell>{row.TaskCompletedBy}</TableCell>
          ...
          <form>
            <div>
              <div className="form-group">
                <label>Select Audit Id</label>
                <select
                  className="form-control"
                  value={selectedAuditId} // Set the selected value based on state
                  onChange={handleSelectChange} // Call the handler on change
                >
                  <option value="" disabled>
                    Select Audit Id
                  </option>
                  {getData.map((row, index) => (
                    <option key={index} value={row.AuditId}>
                      {row.AuditId}
                    </option>
                  ))}
                </select>
