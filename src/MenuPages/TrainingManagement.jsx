...
export default function TrainingManagement() {
  ...
  const dataToSend =
    { id: "65000d1dbc06b751241e74e3", menuname: "trainingdata_1" } || {};
  const pathSubMenu = "/trainingManagement";
  ...
  function fetchData(token) {
    ...
    fetch(
      `http://localhost:8000/v1/collection/trainingdata?page=${currentPage}&limit=100`,
      requestOptions1
    )
  ...
  // Submit data to backend functionality below
  // Submit data to backend functionality below
  // Submit data to backend functionality below
  const handleSubmit = (e) => {
    e.preventDefault();
    ...
    fetch("http://localhost:8000/v1/collection/trainingdata", requestOptions) // Replace with your backend API endpoint
      .then((response) => response.text())
      .then((data) => {
        // console.log(data);
    ...
    <div className="app-main__inner">
      <div className="main-card mb-3 card">
        <div className="card-header">
          ...
          <div className="btn-actions-pane-right" style={{}}>
            <input
              type="text"
                ...
                {getData && getData.length > 0 ? (
                  getData.map((row, index) => (
                    ...
                    >
                      ...
                      {/* <TableCell>{row.trainingDescription}</TableCell> */}
                      {/* <TableCell
                        style={{
                          whiteSpace: "normal",
                          wordWrap: "break-word",
                          width: "40px", // Increase the width to your desired value, e.g., 40%
                        }}
                      >
                        {expandedTitle[index] ? (
                          <>
                          ...
                          <>
                            {row.trainingDescription.slice(0, 40)}{" "}
                            {row.trainingDescription.length > 40 && (
                              <button
                                style={{
                                  backgroundColor: "transparent", // Remove background color
                                  border: "none", // Remove border
                      ...
                      <TableCell>{row.trainingEffectiveFrom}</TableCell>
                      <TableCell>{row.trainingLastDate}</TableCell>
                      <TableCell>{row.trainingStatus}</TableCell>
                      <TableCell>{row.trainingScope}</TableCell>
                      <TableCell>{row.isMandatory}</TableCell>
                      <TableCell>{row.trainingLink}</TableCell>
                      {/* <TableCell>
                        <a
              ...
              <div>
                <form onSubmit={handleSubmit}>
                  ...
                  <input
                    ...
                    value={formData.trainingEffectiveFrom}
                    onChange={handleInputChange}
                  />
                  <input
                    type="text"
                    name="trainingLastDate"
                    ...
                    value={formData.trainingLastDate}
                    onChange={handleInputChange}
                  />
                  <input
                    type="text"
